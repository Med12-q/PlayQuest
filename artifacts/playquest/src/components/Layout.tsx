...
import { Home, Compass, MessageCircle, Bell, User, Settings, Zap, LogOut, Hash } from "lucide-react";
...
const navItems = [
  ...
  { icon: Hash, label: "Chat Public", path: "/chat" },
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0f] flex">
      ...
      <aside className="hidden md:flex flex-col fixed left-0 top-0 h-full w-60 z-50" style={{ background: "rgba(10,10,15,0.97)", backdropFilter: "blur(24px)", borderRight: "1px solid rgba(255,255,255,0.04)" }}>
        ...
        <nav className="flex-1 px-3 py-1 space-y-0.5">
          {navItems.map(({ icon: Icon, label, path }) => {
            const active = location === path || (path !== "/feed" && location.startsWith(path));
            ...
            return (
              <Link key={path} to={path}>
                <motion.div
                  ...
                  style={{
                    background: active ? "rgba(232,16,42,0.08)" : "transparent",
                    color: active ? "#e8102a" : "rgba(232,232,240,0.45)",
                  }}
                >
                  {active && (
                    <div className="absolute left-0 top-1/2 -translate-y-1/2 w-0.5 h-5 rounded-r-full" style={{ background: "#e8102a", boxShadow: "0 0 8px rgba(232,16,42,0.8)" }} />
                  )}
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 z-50 flex items-center justify-around h-14" style={{ background: "rgba(10,10,15,0.98)", backdropFilter: "blur(24px)", borderTop: "1px solid rgba(255,255,255,0.04)" }}>
        {[navItems[0], navItems[1], navItems[2], navItems[3]].map(({ icon: Icon, path }) => {
          const active = location === path || (path !== "/feed" && location.startsWith(path));
          const badge = getBadge(path);
          return (
            <Link key={path} to={path}>
              <div className="relative flex flex-col items-center justify-center w-11 h-11">
                <Icon
                  ...
                  style={{ color: active ? "#e8102a" : "rgba(232,232,240,0.4)", filter: active ? "drop-shadow(0 0 6px rgba(232,16,42,0.7))" : "none" }}
                />
                {badge > 0 && (
